...
object EffectiveGiveCommand : EffectiveCommand() {
    ...
    override fun commandTree() = CommandNode.build {
        executes { args ->
            if (args.size < 2) {
                sendMessage(Locale.getComponent("commands.egive.usage"))
                return@executes
            }
            ...
            if (targets.isEmpty()) {
                sendMessage(Locale.getComponent("commands.egive.player_not_found"))
                return@executes
            }
            val effectiveItem = EffectiveItem.getEffectiveItemByNamespacedKey(args[1])
            if (effectiveItem == null) {
                sendMessage(Locale.getComponent("commands.egive.item_not_found", args[1]))
                return@executes
            }
