...
object EffectiveMobCommand : EffectiveCommand() {
    ...
    override fun commandTree() = CommandNode.build {
        executes { args ->
            if (this !is Player) {
                sendMessage(Locale.getComponent("commands.emob.only_players"))
                return@executes
            }
            if (args.isEmpty()) {
                sendMessage(Locale.getComponent("commands.emob.usage"))
                return@executes
            }
            val entityKey = args[0]
            val effectiveEntity = EffectiveEntity.namespacedKeyToEffectiveEntity[entityKey]
            if (effectiveEntity == null) {
                sendMessage(Locale.getComponent("commands.emob.entity_not_found", entityKey))
                return@executes
            }
            if (entityKey in getCompositePartKeys()) {
                sendMessage(Locale.getComponent("commands.emob.composite_part_not_allowed", entityKey))
                return@executes
            }
            effectiveEntity.spawnEntity(location)
            sendMessage(Locale.getComponent("commands.emob.success", entityKey))
        }
        dynamic {
