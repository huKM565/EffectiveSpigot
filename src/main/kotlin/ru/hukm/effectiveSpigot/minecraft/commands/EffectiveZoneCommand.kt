...
object EffectiveZoneCommand : EffectiveCommand() {
    ...
    override fun commandTree() = CommandNode.build {
        executes { _ ->
            sendMessage(Locale.getComponent("commands.ezone.usage"))
        }
        ...
        choice("create") {
            executes { args ->
                if (this !is Player) {
                    sendMessage(Locale.getComponent("commands.ezone.not_player"))
                    return@executes
                }
                val player = this
                if (args.size < 2) {
                    sendMessage(Locale.getComponent("commands.ezone.create_usage"))
                    sendMessage(Locale.getComponent("commands.ezone.available_zones"))
                ...
                if (selection == null || selection.first == null || selection.second == null) {
                    sendMessage(Locale.getComponent("commands.ezone.no_selection"))
                    return@executes
                }
                val zoneType = args[1]
                ...
                sendMessage(Locale.getComponent("commands.ezone.create_success", zoneType))
            }
        ...
        choice("delete") {
            executes { args ->
                if (args.size < 2) {
                    sendMessage(Locale.getComponent("commands.ezone.delete_usage"))
                    return@executes
                }
                val id = args[1].toIntOrNull()
                if (id == null) {
                    sendMessage(Locale.getComponent("commands.ezone.invalid_id"))
                    return@executes
                }
                if (EffectiveZone.deleteZoneBoxById(id)) {
                    sendMessage(Locale.getComponent("commands.ezone.delete_success", id))
                } else {
                    sendMessage(Locale.getComponent("commands.ezone.not_found"))
                }
            }
            dynamic {
