...
object EffectiveScreenCommand : EffectiveCommand() {
    ...
    override fun commandTree() = CommandNode.build {
        executes { _ ->
            sendMessage(Locale.getComponent("commands.escreen.usage"))
        }
        dynamic({ listOf("@a", "@p") + Bukkit.getOnlinePlayers().map { it.name } }) {
            executes { args ->
                if (args.size < 2) {
                    ...
                    sendMessage(Locale.getComponent("commands.escreen.unknown_effect"))
                }
            }
            choice("fade") {
                executes { args ->
                    ...
                    if (targets.isEmpty()) {
                        sendMessage(Locale.getComponent("commands.escreen.player_not_found"))
                        return@executes
                    }
                    ...
                    sendMessage(Locale.getComponent("commands.escreen.success_fade"))
                }
                dynamic({ listOf(Locale.getMessage("commands.escreen.completions.fade_in")) }) {
                    dynamic({ listOf(Locale.getMessage("commands.escreen.completions.stay")) }) {
                        dynamic { listOf(Locale.getMessage("commands.escreen.completions.fade_out")) }
                    }
                }
            }
            choice("shake") {
                executes { args ->
                    ...
                    sendMessage(Locale.getComponent("commands.escreen.success_shake"))
                }
                dynamic({ listOf(Locale.getMessage("commands.escreen.completions.intensity")) }) {
                    dynamic({ listOf(Locale.getMessage("commands.escreen.completions.duration")) }) {
