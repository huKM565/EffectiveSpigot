...
object EffectiveCompositeCommand : EffectiveCommand() {
    ...
    override fun commandTree() = CommandNode.build {
        executes { args ->
            if (this !is Player) {
                sendMessage(Locale.getComponent("commands.ecomposite.only_players"))
                return@executes
            }
            if (args.isEmpty()) {
                sendMessage(Locale.getComponent("commands.ecomposite.usage"))
                return@executes
            }
            val key = args[0]
            val composite = EffectiveCompositeEntity.namespacedKeyToEffectiveCompositeEntity[key]
            if (composite == null) {
                sendMessage(Locale.getComponent("commands.ecomposite.entity_not_found", key))
                return@executes
            }
            ...
            sendMessage(Locale.getComponent("commands.ecomposite.success", key))
        }
