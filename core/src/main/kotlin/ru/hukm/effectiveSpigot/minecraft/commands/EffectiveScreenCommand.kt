...
object EffectiveScreenCommand : EffectiveCommand() {
    ...
    override fun commandTree() = CommandNode.build {
        executes { _ ->
            sendMessage(Locale.getMessage("commands.escreen.usage"))
        }
        dynamic({ listOf("@a", "@p") + Bukkit.getOnlinePlayers().map { it.name } }) {
            executes { args ->
                if (args.size < 2) {
                    ...
                    sendMessage(Locale.getMessage("commands.escreen.unknown_effect"))
                }
            }
            choice("fade") {
                executes { args ->
                    ...
                    if (targets.isEmpty()) {
                        sendMessage(Locale.getMessage("commands.escreen.player_not_found"))
                        return@executes
                    }
                    ...
                    sendMessage(Locale.getMessage("commands.escreen.success_fade"))
                }
            ...
            choice("shake") {
                executes { args ->
                    ...
                    sendMessage(Locale.getMessage("commands.escreen.success_shake"))
                }
